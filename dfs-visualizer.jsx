...
const PSEUDOCODE = [
  { id: 0, indent: 0, text: "DFS(graph, start):" },
  { id: 1, indent: 1, text: "visited ← { }" },
  { id: 2, indent: 1, text: "stack ← [ start ]" },
  { id: 3, indent: 1, text: "order ← []" },
  { id: 4, indent: 1, text: "" },
  { id: 5, indent: 1, text: "while stack is not empty:" },
  { id: 6, indent: 2, text: "node ← stack.pop()" },
  { id: 7, indent: 2, text: "" },
  { id: 8, indent: 2, text: "if node in visited:" },
  { id: 9, indent: 3, text: "continue" },
  { id: 10, indent: 2, text: "" },
  { id: 11, indent: 2, text: "visited.add(node)" },
  { id: 12, indent: 2, text: "order.append(node)" },
  { id: 13, indent: 2, text: "" },
  { id: 14, indent: 2, text: "for neighbor in graph[node]:" },
  { id: 15, indent: 3, text: "if neighbor not in visited:" },
  { id: 16, indent: 4, text: "stack.push(neighbor)" },
  { id: 17, indent: 1, text: "" },
  { id: 18, indent: 1, text: "return order" },
];
...
function generateDFSSteps(graph, start) {
  ...
  steps.push({
    ...
    currentNeighbor: null,
    processingEdge: null,
    description: `Initialize: visited = {}, stack = [${start}]`,
    phase: "init",
  });
  ...
  while (stack.length > 0) {
    ...
    steps.push({
      ...
      currentNeighbor: null,
      processingEdge: null,
      description: `Pop "${node}" from stack`,
      phase: "pop",
    });
    ...
    if (visited.has(node)) {
      steps.push({
        ...
        currentNeighbor: null,
        processingEdge: null,
        description: `"${node}" already visited → skip (continue)`,
        phase: "skip",
      });
      continue;
    }
    ...
    steps.push({
      ...
      currentNeighbor: null,
      processingEdge: null,
      description: `Mark "${node}" visited, append to traversal order`,
      phase: "visit",
    });
    ...
    for (const neighbor of neighbors) {
      if (!visited.has(neighbor)) {
        ...
        steps.push({
          ...
          currentNeighbor: neighbor,
          processingEdge: [node, neighbor],
          description: `Push "${neighbor}" onto stack`,
          phase: "push",
        });
      } else {
        steps.push({
...
const allSteps = generateDFSSteps(GRAPH, "A");
...
export default function DFSVisualizer() {
  ...
  const getNodeColor = (node) => {
    ...
    if (current.stack.includes(node) && !current.visited.has(node)) return colors.stacked;
  ...
  const getNodeBg = (node) => {
    ...
    if (current.stack.includes(node) && !current.visited.has(node)) return colors.stackedSoft;
    if (current.visited.has(node)) return colors.accentSoft;
    return "transparent";
  };

  const getEdgeColor = (a, b) => {
    if (current.processingEdge &&
    ...
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: 24 }}>
        ...
        <p style={{ color: colors.textDim, fontSize: 12, margin: "6px 0 0" }}>
          Interactive algorithm visualizer · Stack-based iterative approach
        </p>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          borderRadius: 10, padding: "18px 0", overflow: "hidden",
        }}>
          <div style={{
            fontSize: 10, fontWeight: 600, textTransform: "uppercase",
            letterSpacing: "0.12em", color: colors.textDim,
            padding: "0 18px 12px", borderBottom: `1px solid ${colors.border}`, marginBottom: 8,
          }}>
            Pseudocode
          </div>
            ...
            {PSEUDOCODE.map((line) => {
              ...
              return (
                <div key={line.id} style={{
                  ...
                  <span style={{
                    color: colors.textDim, fontSize: 10, width: 22,
                    textAlign: "right", marginRight: 10, opacity: 0.5, flexShrink: 0,
                  }}>
                    {line.text ? line.id : ""}
                  </span>
                  <span style={{
                    paddingLeft: line.indent * 20, fontSize: 13,
                    color: isHighlighted ? colors.highlight : colors.text,
                    fontWeight: isHighlighted ? 600 : 400, transition: "color 0.2s",
                  }}>
                    {line.text}
                  </span>
                </div>
        ...
        <div style={{
          ...
          <svg viewBox="-10 10 420 360" style={{ width: "100%", flex: 1 }}>
            {EDGES.map(([a, b]) => (
              <line key={`${a}-${b}`}
                ...
                stroke={getEdgeColor(a, b)} strokeWidth={getEdgeWidth(a, b)}
            ...
            {Object.entries(NODE_POSITIONS).map(([node, pos]) => (
              <g key={node}>
                <circle cx={pos.x} cy={pos.y} r={26}
                  fill={getNodeBg(node)} stroke={getNodeColor(node)}
                  strokeWidth={current.currentNode === node ? 2.5 : 1.5}
                  style={{ transition: "all 0.3s ease" }}
                />
                <text x={pos.x} y={pos.y + 1} textAnchor="middle"
                  dominantBaseline="central" fill={getNodeColor(node)}
                ...
                {current.order.indexOf(node) !== -1 && (
                  <g>
                    <circle cx={pos.x + 20} cy={pos.y - 20} r={10} fill={colors.accent} />
                    <text x={pos.x + 20} y={pos.y - 19} textAnchor="middle"
                      dominantBaseline="central" fill={colors.bg} fontSize={10}
          ...
          <div style={{
            ...
            {[
              ...
              { label: "In Stack", color: colors.stacked },
      ...
      {/* State Panel */}
      <div style={{
        maxWidth: 960, margin: "18px auto 0",
        display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14,
      }}>
        {[
          { label: "Stack (top →)", items: [...current.stack].reverse(), color: colors.stacked, bgColor: colors.stackedSoft },
      ...
      {/* Description */}
      <div style={{
        maxWidth: 960, margin: "14px auto 0",
      ...
      <div style={{
        ...
        <button onClick={() => setStep((s) => Math.min(allSteps.length - 1, s + 1))}
          ...
          style={{
            ...
            border: `1px solid ${colors.border}`, borderRadius: 6,
            padding: "8px 14px", fontSize: 13,
            cursor: step === allSteps.length - 1 ? "not-allowed" : "pointer", fontFamily: "inherit",
          }}>▶</button>
        <div style={{ marginLeft: 16, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {[
            ...
            <button key={label} onClick={() => setSpeed(val)} style={{
              ...
              cursor: "pointer", fontFamily: "inherit", fontWeight: speed === val ? 600 : 400,
            }}>{label}</button>
          ))}
        </div>
      </div>
      ...
      {/* Step scrubber */}
      <div style={{ maxWidth: 960, margin: "14px auto 0", padding: "0 4px" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 11, fontWeight: 600, color: colors.accent, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.08em" }}>
          Key Difference from BFS
        </div>
        <div style={{ fontSize: 12, color: colors.textDim, lineHeight: 1.6 }}>
          DFS uses a <span style={{ color: colors.stacked, fontWeight: 600 }}>stack</span> (LIFO) instead of a queue (FIFO).
          This means it explores as <em>deep</em> as possible along each branch before backtracking,
          while BFS explores all neighbors at the current depth first. Notice how the traversal order
          dives down one path before visiting siblings.
        </div>
      </div>
    </div>
  );
